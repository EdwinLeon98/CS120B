...
unsigned char TickFunc(unsigned char counter, unsigned char A0, unsigned char A1, unsigned char A2, unsigned char A7, unsigned char B){
	switch(SM_State){
		case SM_Start:
			SM_State = SM_Wait;
			break;
		...
		case SM_Code: 
                        ...
                        else if(!A0 && !A1 && !A2 && !A7 && ((counter / 2) < 3)){
                                SM_State = SM_WaitCode;
                        }
			else if(!A0 && !A1 && !A2 && !A7 && !((counter / 2) < 3)){
...
int main(void) {
    ...
    unsigned char CorrectArray[] = {0x01, 0x02, 0x01};
	...
	tempB = TickFunc(counter, tempA0, tempA1, tempA2, tempA7, tempB);

	if(SM_State == SM_Code){
		tempArray[counter / 2] = tempB;
		counter = counter + 1;
	}

	if((counter / 2) == 3){
		for(x = 0; x < 3; x++){
			if(tempArray[x] == CorrectArray[x]){
				y = y + 1;
			}
		}
		if(y != 3){
	...
	if(SM_State == SM_Wait || SM_State == SM_Wait2){
		counter = 0;
	}
